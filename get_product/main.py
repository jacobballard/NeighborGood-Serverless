...
def get_product(request: flask.Request):
    ...
    modifiers = [dict(zip(row.keys(), row)) for row in modifiers_data]

    # Get modifier_details for each modifier
    for modifier in modifiers:
        if modifier['modifier_type'] == 'choice':
            details_query = text('SELECT * FROM modifier_details WHERE modifier_id = :modifier_id')
            details_data = connection.execute(details_query, {'modifier_id': modifier['id']}).fetchall()
            modifier['options'] = [dict(zip(row.keys(), row)) for row in details_data]
